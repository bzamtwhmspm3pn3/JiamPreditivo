...
gerar_datas_previsao_robusto <- function(parametros, n_previsoes, freq, tem_datas = FALSE, ultima_data_real = NULL) {
  ...
  if (!is.null(parametros$periodo_inicio) && parametros$periodo_inicio != "") {
    cat("📅 [ETS] Usando período início dos parâmetros:", parametros$periodo_inicio, "\n")
    ...
    tryCatch({
      ...
      if (grepl("^\\d{1,2}/\\d{4}$", periodo_str)) {
        ...
        cat("📅 [ETS] Mês inicio ajustado:", mes_inicio, "\n")
        cat("📅 [ETS] Ano inicio ajustado:", ano_inicio, "\n")
        ...
        cat("📅 [ETS] Datas geradas:", paste(head(datas_previsao, 5), collapse = ", "), 
            if (n_previsoes > 5) paste("...", tail(datas_previsao, 1)) else "", "\n")
        
        return(as.list(datas_previsao))
        
      } else {
        cat("⚠️ [ETS] Formato de período início inválido:", periodo_str, "\n")
      }
    }, error = function(e) {
      cat("❌ [ETS] Erro ao processar período início:", e$message, "\n")
    })
  }
  
  # ✅ FALLBACK: Períodos numéricos
  cat("📅 [ETS] Usando períodos numéricos como fallback\n")
  return(as.list(paste("Período", 1:n_previsoes)))
}

# ✅ Função para calcular métricas avançadas de precisão para ETS
calcular_metricas_avancadas_ets <- function(modelo, ts_data, previsoes, n_previsoes, freq) {
  ...
  valores_validos <- valores_ajustados[!is.na(residuos) & !is.na(valores_ajustados)]
  residuos_validos <- residuos_clean[1:min(length(residuos_clean), length(valores_validos))]
  
  if (length(valores_validos) > 0 && all(valores_validos != 0)) {
    mape <- mean(abs(residuos_validos / valores_validos), na.rm = TRUE) * 100
  } else {
    mape <- NA
  }
  ...
  parametros_suavizacao <- list()
  coefs <- coef(modelo)
  
  if (!is.null(coefs)) {
    if ("alpha" %in% names(coefs)) parametros_suavizacao$alpha <- as.numeric(coefs["alpha"])
    if ("beta" %in% names(coefs)) parametros_suavizacao$beta <- as.numeric(coefs["beta"])
    if ("gamma" %in% names(coefs)) parametros_suavizacao$gamma <- as.numeric(coefs["gamma"])
    if ("phi" %in% names(coefs)) parametros_suavizacao$phi <- as.numeric(coefs["phi"])
    if ("l" %in% names(coefs)) parametros_suavizacao$nivel <- as.numeric(coefs["l"])
    if ("b" %in% names(coefs)) parametros_suavizacao$tendencia <- as.numeric(coefs["b"])
    if ("s1" %in% names(coefs)) parametros_suavizacao$sazonal_1 <- as.numeric(coefs["s1"])
  }
  
  # Estados finais
  estados_finais <- tryCatch({
    if (!is.null(modelo$states)) {
      as.numeric(tail(modelo$states, 1))
    } else {
      NULL
    }
  ...
  return(list(
    ...
    parametros_suavizacao = parametros_suavizacao,
...
# ✅ Função principal para processar resultados ETS
processar_resultado_ets_profissional <- function(modelo, ts_data, n_previsoes, parametros, 
                                                modelo_ets, periodo, y_var) {
  ...
  tryCatch({
    ...
    cat("⚠️ [ETS] Usando intervalo de confiança padrão (80%, 95%)\n")
  ...
  metricas_avancadas <- calcular_metricas_avancadas_ets(modelo, ts_data, previsoes, n_previsoes, freq)
  ...
  # ✅ Descrição dos componentes ETS
  descricao_erro <- switch(substr(modelo_ets, 1, 1),
    "A" = "Aditivo",
    "M" = "Multiplicativo",
    "Z" = "Automático",
    "N" = "Nenhum",
  ...
  descricao_tendencia <- switch(substr(modelo_ets, 2, 2),
    "A" = "Aditiva",
    "M" = "Multiplicativa",
    "Z" = "Automática",
    "N" = "Nenhuma",
    "Ad" = "Atenuada Aditiva",
    "Md" = "Atenuada Multiplicativa",
    "Nenhuma"
  )
  
  descricao_sazonal <- switch(substr(modelo_ets, 3, 3),
    "A" = "Aditiva",
    "M" = "Multiplicativa",
    "Z" = "Automática",
    "N" = "Nenhuma",
  ...
  resultado <- list(
    ...
    modelo_info = list(
      tipo = "ETS",
      especificacao = modelo_ets,
      periodo_sazonal = periodo,
      componentes = list(
        erro = substr(modelo_ets, 1, 1),
        tendencia = substr(modelo_ets, 2, 2),
        sazonalidade = substr(modelo_ets, 3, 3)
      ),
      descricao = paste(
        "Erro:", descricao_erro,
        "| Tendência:", descricao_tendencia,
        "| Sazonalidade:", descricao_sazonal
      ),
      metodo = "Suavização exponencial",
      frequencia = freq,
      log_likelihood = metricas_avancadas$log_likelihood,
      sigma2 = metricas_avancadas$sigma2
    ),
    
    # Métricas
    metricas = list(
      ajuste = list(
        RMSE = metricas_avancadas$RMSE,
        MAE = metricas_avancadas$MAE,
        MAPE = metricas_avancadas$MAPE,
        AIC = metricas_avancadas$AIC,
        BIC = metricas_avancadas$BIC,
        AICc = metricas_avancadas$AICc,
        R2 = metricas_avancadas$R2,
        R2_adj = metricas_avancadas$R2_adj,
        Theil_U = metricas_avancadas$Theil_U,
        MASE = metricas_avancadas$MASE,
        sMAPE = metricas_avancadas$sMAPE
      ),
      
      suavizacao = metricas_avancadas$parametros_suavizacao,
      
      estados_finais = metricas_avancadas$estados_finais,
      ...
      diagnostico = list(
        ...
        # Estatísticas dos resíduos
        residuos_estatisticas = list(
          media = metricas_avancadas$residuos_mean,
          desvio_padrao = metricas_avancadas$residuos_sd,
          assimetria = metricas_avancadas$residuos_skewness,
          curtose = metricas_avancadas$residuos_kurtosis
        )
      )
    ),
    ...
    qualidade_ajuste = list(
      ...
      classificacao_mase = if (!is.na(metricas_avancadas$MASE)) {
        if (metricas_avancadas$MASE < 1) "Melhor que naive" else
        if (metricas_avancadas$MASE < 1.5) "Aceitável" else "Precária"
      } else "N/A",
      ...
      recomendacoes = if (n_outliers > 0) {
        ...
        c("Autocorrelação nos resíduos", "Considerar modelo com mais componentes")
      } else if (!is.null(shapiro_test) && shapiro_test$p.value < 0.05) {
        c("Resíduos não normais", "Verificar transformações da série")
      } else {
    ...
    # Componentes do modelo ETS
    componentes = list(
      nivel_final = if (!is.null(metricas_avancadas$estados_finais)) {
        metricas_avancadas$estados_finais[1]
      } else NA,
      
      tendencia_final = if (!is.null(metricas_avancadas$estados_finais) && length(metricas_avancadas$estados_finais) > 1) {
        metricas_avancadas$estados_finais[2]
      } else NA,
      
      sazonal_final = if (!is.null(metricas_avancadas$estados_finais) && length(metricas_avancadas$estados_finais) > 2) {
        metricas_avancadas$estados_finais[3]
      } else NA,
      
      interpretacao_suavizacao = if (!is.null(metricas_avancadas$parametros_suavizacao$alpha)) {
        alpha <- metricas_avancadas$parametros_suavizacao$alpha
        paste0("Alpha = ", round(alpha, 4), 
               if (alpha < 0.1) " (suavização forte)" else 
               if (alpha < 0.3) " (suavização moderada)" else " (suavização leve)")
      } else NULL
    )
  )
...
main <- function() {
  ...
  if (length(args) < 2) {
    stop("Uso: Rscript ets.R <input.json> <output.json>")
  }
  ...
  tryCatch({
    cat("🚀 Iniciando ETS Profissional\n")
    ...
    # Determinar modelo ETS
    erro_param <- parametros$erro %||% "Z"
    tendencia_param <- parametros$tendencia %||% "Z"
    sazonalidade_param <- parametros$sazonalidade %||% "Z"
    periodo <- as.integer(parametros$periodo %||% freq)
    
    # Criar string do modelo ETS
    modelo_ets <- paste0(erro_param, tendencia_param, sazonalidade_param)
    
    cat("⚙️  Configurando ETS:", modelo_ets, "[m=", periodo, "]\n")
    ...
    # Atualizar modelo_ets com o modelo real ajustado
    modelo_ets_ajustado <- modelo$method
    
    # Processar resultados
    resultado <- processar_resultado_ets_profissional(
      modelo, ts_data, n_previsoes, parametros, 
      modelo_ets_ajustado, periodo, y_var
    )
    
    # Adicionar metadados
    resultado$metadados <- list(
      versao_modelo = "ETS Profissional 2.0",
      timestamp = Sys.time(),
    ...
    cat("✅ Modelo ETS executado com sucesso!\n")
